angular.module('newApp')
    .controller('productsCtrl', ['$scope', '$location', 'pluginsService', '$http', 'config', function ($scope, $location, pluginsService, $http, config) {




        //HTML AUX OBJECTS
        $scope.header = ['Medicine', 'Pharmaceutical House', 'Quantity'];
        $scope.graph = {};

        $scope.items = [];
        $http.get(config.ip + '/GasStationPharmacy/api/Statistics/1')
            .success(function (result) {
                ...
                $scope.graphData = JSON.parse(JSON.stringify($scope.items).split('"medicine":').join('"label":'));
        ...
        $scope.fillGraph = function () {
            $scope.graph = {
                chart: {
                    ...
                    subCaption: "Most Sold Products in total",
            ...
            console.log(JSON.stringify($scope.graph))
        }
