angular.module('newApp')
    .controller('subsidiaryCtrl', ['$scope', '$location', 'config', '$http', function ($scope, $location, config, $http) {
        ...
        //companies
        $scope.companies = [];
        $http.get(config.ip + '/GasStationPharmacy/api/Companies')
        ...
        //admins
        $scope.admins = [];
        $http.get(config.ip + '/GasStationPharmacy/api/Administrators')
            .success(function (result) {
                $scope.admins = result;
        ...
        //employees
        $scope.employees = [];
        $http.get(config.ip + '/GasStationPharmacy/api/Employees')
            .success(function (result) {
        ...
        $scope.save = function () {
            ...
            if ($scope.checkUnchangedFields.apply(this, arguments)) {
                ...
                $http.put(config.ip + '/GasStationPharmacy/api/Subsidiaries/' + $scope.items[i].id_subsidiary, put_resquest)
                    .success(function (result) {
                        console.log(result);
                    })
                    .error(function (data, status) {
        ...
        $scope.verifiedAdd = function () {
            ...
            $http.post(config.ip + '/GasStationPharmacy/api/Subsidiaries', post_resquest)
                .success(function (result) {
                    console.log(result);
                })
                .error(function (data, status) {
        ...
        $scope.remove = function (index, index_aux) {
            ...
            if (confirm("Are you sure to delete this row?") == true) {
                ...
                $http.delete(config.ip + '/GasStationPharmacy/api/Subsidiaries/' + $scope.items[i].id_subsidiary)
                    .success(function (result) {
                        console.log(result);
                    })
                    .error(function (data, status) {
        ...
        $scope.items = [];
        $http.get(config.ip + '/GasStationPharmacy/api/Subsidiaries')
            .success(function (result) {
                $scope.items = result;
                $scope.pageRecalc();
