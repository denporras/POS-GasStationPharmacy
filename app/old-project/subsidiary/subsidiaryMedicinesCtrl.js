angular.module('newApp')
    .controller('subsidiaryMedicinesCtrl', ['$scope', '$location', 'config', '$http', '$routeParams', function ($scope, $location, config, $http, $routeParams) {
        ...
        //Medicines
        $scope.medicines = [];
        $http.get(config.ip + '/GasStationPharmacy/api/Medicines')
        ...
        $scope.save = function () {
            ...
            if ($scope.checkUnchangedFields.apply(this, arguments)) {
                ...
                $http.put(config.ip + '/GasStationPharmacy/api/Subsidiaries/' + $scope.id, $scope.subsidiary_put_request)
                    .success(function (result) {
                        console.log(result);
                    })
                    .error(function (data, status) {
        ...
        $scope.verifiedAdd = function () {
            ...
            //*******EDIT POST******/
            $http.post(config.ip + '/GasStationPharmacy/api/MedicinesbySubsidiary', post_resquest)
                .success(function (result) {
                    console.log(result);
                })
                .error(function (data, status) {
        ...
        $scope.items = [];
        $scope.subsidiary = {};
        $http.get(config.ip + '/GasStationPharmacy/api/Subsidiaries/'+$scope.id)
            .success(function (result) {
