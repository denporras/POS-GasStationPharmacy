angular.module('newApp')
    .controller('doctorsCtrl', ['$scope', '$location', 'config', '$http', function ($scope, $location, config, $http) {
        ...
        //roles
        $scope.roles = [];
        $http.get(config.ip + '/GasStationPharmacy/api/Roles')
            .success(function (result) {
                $scope.roles = result;
        ...
        $scope.save = function () {
            ...
            if ($scope.checkUnchangedFields.apply(this, arguments)) {
                ...
                $http.put(config.ip + '/GasStationPharmacy/api/Employees/' + $scope.items[i].id_employee, put_resquest)
                    .success(function (result) {
                        console.log(result);
                    })
                    .error(function (data, status) {
        ...
        $scope.verifiedAdd = function () {
            ...
            $http.post(config.ip + '/GasStationPharmacy/api/Employees', post_resquest)
                .success(function (result) {
                    console.log(result);
                })
                .error(function (data, status) {
        ...
        $scope.remove = function (index, index_aux) {
            ...
            if (confirm("Are you sure to delete this row?") == true) {
                ...
                $http.delete(config.ip + '/GasStationPharmacy/api/Employees/' + $scope.items[i].id_employee)
                    .success(function (result) {
                        console.log(result);
                    })
                    .error(function (data, status) {
