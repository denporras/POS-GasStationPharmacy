angular.module('newApp')
    .controller('drugsCtrl', ['$scope', '$location', 'pluginsService', '$http', 'config', function ($scope, $location, pluginsService, $http, config) {
        ...
        //houses
        $scope.houses = [];
        $http.get(config.ip + '/GasStationPharmacy/api/PharmaceuticalHouses')
        ...
        $scope.save = function () {
            ...
            if ($scope.checkUnchangedFields.apply(this, arguments)) {
                ...
                $http.put(config.ip + '/GasStationPharmacy/api/Medicines/' + $scope.items[i].id_medicine, put_resquest)
                    .success(function (result) {
                        console.log(result);
                    })
                    .error(function (data, status) {
        ...
        $scope.verifiedAdd = function () {
            ...
            $http.post(config.ip + '/GasStationPharmacy/api/Medicines', post_resquest)
                .success(function (result) {
                    console.log(result);
                })
                .error(function (data, status) {
        ...
        $scope.remove = function (index, index_aux) {
            ...
            if (confirm("Are you sure to delete this row?") == true) {
                ...
                $http.delete(config.ip + '/GasStationPharmacy/api/Medicines/' + $scope.items[i].id_medicine)
                    .success(function (result) {
                        console.log(result);
                    })
                    .error(function (data, status) {
