    ...
    .controller('clientsCtrl', ['$scope', '$location', 'pluginsService', '$http', 'config', function ($scope, $location, pluginsService, $http, config) {
        ...
        $scope.save = function () {
            ...
            if ($scope.checkUnchangedFields.apply(this, arguments)) {
                ...
                $http.put(config.ip + '/GasStationPharmacy/api/Clients/' + $scope.items[i].id_client, put_resquest)
        ...
        $scope.verifiedAdd = function () {
            ...
            $http.post(config.ip + '/GasStationPharmacy/api/Clients', post_resquest)
                .success(function (result) {
                    console.log(result);
                })
                .error(function (data, status) {
        ...
        $scope.remove = function (index, index_aux) {
            ...
            if (confirm("Are you sure to delete this row?") == true) {
                ...
                $http.delete(config.ip + '/GasStationPharmacy/api/Clients/' + $scope.items[i].id_client)
                    .success(function (result) {
                        console.log(result);
                    })
                    .error(function (data, status) {
        ...
        $scope.items = [];
        $http.get(config.ip + '/GasStationPharmacy/api/Clients')
            .success(function (result) {
                $scope.items = result;
